...
---
# Войдите в контейнер от root через pct enter (как показано выше)

# Удаляем старого пользователя (если нужно)
userdel -r aaa

# Создаем пользователя с домашней директорией и правами sudo
useradd -m -G sudo -s /bin/bash aaa
passwd aaa  # установите новый пароль
